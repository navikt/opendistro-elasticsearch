...
if [ -f /usr/share/elasticsearch/config/gcs-key.json ]; then
  echo "using gcs key"
  /usr/share/elasticsearch/bin/elasticsearch-keystore add-file -f gcs.client.default.credentials_file /usr/share/elasticsearch/config/gcs-key.json
fi
